-- ============================================================
-- Nägels Online — Review Fixes for 002_sync_redesign
--
-- Code-quality reviewer flagged:
--   I1: rooms.host_session_id ON DELETE RESTRICT will block user
--       account deletion. Change to CASCADE — deleting the host
--       deletes their rooms (acceptable for a casual game).
--   M1: get_room_state.players returns NULL for empty rooms.
--       Add COALESCE so clients always get an array.
--   M6/M7: idx_rooms_code and idx_room_sessions_auth_user are
--       redundant with their UNIQUE constraints. Drop them.
-- ============================================================

BEGIN;

-- I1 — host_session_id: RESTRICT → CASCADE
ALTER TABLE public.rooms
  DROP CONSTRAINT rooms_host_session_id_fkey,
  ADD  CONSTRAINT rooms_host_session_id_fkey
       FOREIGN KEY (host_session_id)
       REFERENCES public.room_sessions(id)
       ON DELETE CASCADE;

-- M6, M7 — drop redundant indexes
DROP INDEX IF EXISTS public.idx_rooms_code;
DROP INDEX IF EXISTS public.idx_room_sessions_auth_user;

-- M1 — COALESCE players to '[]' instead of NULL when empty
