...
- **Seamless invite → same room (auto-room from Discord instance)** — Discord's native "+" (`openInviteDialog`) DOES notify a friend to join the voice channel + Activity, but they land as a SEPARATE session and must manually join the same Nägels room by code. To make "invite → instantly playing together" seamless, use the Discord Activity **instance** (instance ID + `getInstanceConnectedParticipants` / participant-update events) to auto-create/join ONE shared Nägels room per voice channel. This is the deferred "auto-room from voice channel" idea, now concrete. Our own Share / spectator-link button is the WRONG tool for Discord (it's a web URL, not a Discord invite) — being removed.
- **Discord identity polish (post-auth, 2026-06-15 feedback)** — three small items: (a) REMOVE our custom `game-btn-discord-invite` button — redundant with Discord's native "+" invite (and it only rendered in MP rooms, never in bot games, which confused testing); (b) hide the in-game logout/sign-out affordance inside Discord (identity is Discord-bound — you log out by leaving Discord), gate the sign-out in ProfileScreen/SettingsBody/AuthModal/WelcomeScreen with `isDiscordActivity()`; (c) propagate the Discord avatar everywhere — `discord-auth` already stores `avatar_url` in user_metadata, but `bootstrap.runAuth` only sets `authStore.displayName`, not the avatar, so realtime presence (`realtimeBroadcast.ts` avatar/avatarUrl/avatarColor, sourced from user_metadata via `authService`) still broadcasts the old guest avatar → scoreboard/seats/tooltips show no Discord avatar. Wire the avatar from `getDiscordProfile()` like the name; verify `cdn.discordapp.com` images load inside the Activity CSP.
...
### Хост: «Заморозить комнату» бросает в лобби комнаты, а не на главную (Akula via Discord testing, 2026-06-15)

  - defaultExpanded: false
    ```md
    Хост жмёт «Заморозить комнату» и попадает в **лобби комнаты** (WaitingRoom), а не на **главную страницу** (общее лобби). Чтобы уйти на главную, хосту приходится ещё раз жать выход из комнаты — но выход из лобби комнаты **закрывает/уничтожает комнату**, что противоречит самому смыслу заморозки (комната должна СОХРАНИТЬСЯ и быть возобновляемой). Неприемлемо.

    Ожидается: нажатие «Заморозить комнату» уводит хоста на **главное лобби** (главную страницу), а комната остаётся в состоянии `paused` (frozen) и возобновляемой — без промежуточного захода в WaitingRoom и без её закрытия.

    Связано с фичей заморозки («Заморозка партии хостом — pause & resume», In Progress) и с соседним багом про пост-игровой Leave хоста. Где смотреть: пост-freeze навигация (`freezeWithConfirm` / `gameClient.pauseGame` → куда роутит после паузы) — должна вести в Lobby, а не оставлять/ре-входить в комнату. Найдено при тестировании в Discord, но баг общий (не Discord-специфичный).
    ```
