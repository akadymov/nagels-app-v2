...
### Инструкции для оффлайн игры

  - defaultExpanded: false
    ```md
    Добавить инструкции с визуальными динамическими подсказками - что делать в оффлайн игре между раундами ввода ставок (как рассаживаться, кто и как раздает карты, какой козырь, кто ходит первым, краткие правила и проч.). Инструкции персонализированы, в зависимости от рассадки и текущего порядка ходов.
    ```

### Change the copy above the number of player selector
