...
### Затемнять нелегальные карты в руке в момент хода (Akula, 2026-06-07)

  - defaultExpanded: false
    ```md
    Идея (Akula): когда наступает ход игрока, карты в его руке, которыми нельзя пойти по правилам (нужно ходить в масть / обязан перебить козырем), показывать более блёклыми — приглушённый цвет / пониженная opacity. Чтобы сразу, без раздумий, было визуально понятно, какие карты кликабельны. Эффект только В МОМЕНТ СВОЕГО ХОДА; вне хода (или когда легальна вся рука) — рука в обычном виде, ничего не гасим.

    Хорошая новость — почти вся логика уже есть, это в основном presentational-правка:
    - Набор легальных карт уже вычисляется: `GameTableScreen.tsx:768` `playableCards` (через `engineGetPlayableCards` / `isCardPlayable` из движка правил) и прокидывается в руку: `GameTableScreen.tsx:1679` `playableCards={playableCards.map(c => c.id)}` в `<CardHand>`.
    - Нужно: в `CardHand` (и/или карточном компоненте) карты, чьего id нет в `playableCards`, рендерить с dim-стилем (например opacity ~0.4 + лёгкая десатурация), вместо/в дополнение к текущему disabled-поведению тапа.
    - Гейт «только в свой ход»: применять dim лишь когда сейчас реально ход этого игрока (есть активная фаза игры и `playableCards` — это подмножество руки, а не вся рука). Проверить, чем `playableCards` заполняется вне своего хода (полная рука vs пусто), чтобы не гасить всё/ничего ошибочно.

    Нюансы для проработки: доступность (не только цветом — opacity + чуть меньший contrast, чтобы дальтоникам тоже читалось); анимация перехода dim↔normal при смене хода (transform/opacity, 60fps — см. mobile-first rules); согласовать с уже существующей подсветкой выбранной карты (`selectedCards`) и с «активный ход» подсветкой стола, чтобы не конфликтовали визуально.
    ```
